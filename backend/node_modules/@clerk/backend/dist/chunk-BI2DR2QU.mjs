...
var API_VERSION = "v1";
var USER_AGENT = `${"@clerk/backend"}@${"1.13.2"}`;
...
${error.getFullMessage()}`
  ...
  async function refreshToken(authenticateContext2) {
    ...
    if (!expiredSessionToken || !refreshToken2) {
  ...
  async function attemptRefresh(authenticateContext2) {
    let sessionToken;
    try {
      sessionToken = await refreshToken(authenticateContext2);
    } catch (err) {
  ...
  function handleMaybeHandshakeStatus(authenticateContext2, reason, message, headers, refreshError) {
    if (isRequestEligibleForHandshake(authenticateContext2)) {
      refreshError = refreshError || (authenticateContext2.refreshTokenInCookie ? "non-eligible" : "no-cookie");
  ...
  async function handleError(err, tokenCarrier) {
    ...
    let refreshError = authenticateContext.refreshTokenInCookie ? "non-eligible" : "no-cookie";
...
//# sourceMappingURL=chunk-BI2DR2QU.mjs.map